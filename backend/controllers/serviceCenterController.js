  ...
  create: async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        // Modify GenericModel to accept a connection or pass it in a specific way,
        // or re-implement serviceCenterModel.create here using the connection.
        // For simplicity and direct control within transaction, we'll do it manually here.
        ...
        if (err.code === 'ER_DUP_ENTRY' && err.message.includes('proprietorMobile')) {
      ...
      const DEFAULT_USER_PASSWORD = '1234'; // IMPORTANT: HASH PASSWORDS in production!

      try {
        console.log("in create method of user creation");
        // 2. Create a default user for this service center
        const insertUserQuery = `
          INSERT INTO users (username, password, scId, firstLoginDone)
          VALUES (?, ?, ?, 0);
        `;
        const [userResult] = await connection.execute(insertUserQuery, [proprietorMobile, DEFAULT_USER_PASSWORD, newServiceCenterId]); // Use connection.execute
        ...
        res.status(201).json({
          ...
          // Only for development/admin info, avoid sending password in production
        ...
        if (userErr.code === 'ER_DUP_ENTRY' && userErr.message.includes('username')) {
          return res.status(409).json({ msg: 'A user with this mobile number already exists.' });
        }
