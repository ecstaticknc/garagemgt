...
// Make sure List is imported from react-native-paper
...
  console.log("User Service Center ID:", userScId); // This is fine for debugging
...
    setError(null);
    try {
      const response = await API._get(`/customers?scId=${userScId}`);
...
          onPress: async () => {
            try {
              await API._delete(`/customers/${id}`); // Corrected: Use API._delete
...
      right={props => (
        <List.Item.Actions> {/* CHANGE THIS LINE: Use List.Item.Actions */}
...
        }
        ListEmptyComponent={<Text style={styles.emptyList}>No customers found for this service center.</Text>}
      />
      <FAB
        style={styles.fab}
        icon="plus"
